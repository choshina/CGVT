...
abstract class Simulation (logName: String, verbose: Boolean) {
  ...
  private def conditionalInterleave[T](ls: List[List[T]]) (pred : T => Boolean) : List[List[T]] = {
    // a pure interleaving of lists, preserving only the order in every single list
    val lss = ls.filter((x) => x != Nil) 
    lss.length match {
      case 0 => Nil
      case 1 => List(lss(0))
      case 2 => {
	val l1 = if (pred(lss(0).head)) (conditionalInterleave(List(lss(0).tail, lss(1))) (pred)).map(x => lss(0).head :: x) else Nil
	val l2 = if (pred(lss(1).head)) (conditionalInterleave(List(lss(0), lss(1).tail)) (pred)).map(x => lss(1).head :: x) else Nil
	l1 ::: l2
      }
      case _ => {
	def removeNthHead (n: Int) = { lss.slice(0,n).toList ::: (lss(n).tail :: lss.slice(n+1, lss.length).toList) }
	val heads = (0 until lss.length).toList.map( i => (lss(i).head, removeNthHead(i))) 
	def addHead (x: (T, List[List[T]])) = { if (pred(x._1)) (conditionalInterleave(x._2) (pred)).map( (l) => x._1 :: l) else Nil }
	heads.flatMap(addHead)
      }
    }
  }
  ...
  // def temproalInterleave(tr: List[(Int, Int)]) : List[List[(Int, Int)]] = {
  //   val tdNum = methodLog.length 
  //   val tdTrace = tr.groupBy( x => x._1 ).values.toList
  // }
