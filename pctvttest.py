...
#        f.write("import " + test.importpath + " ; \n")
    ...
    f.write("    public void setScheduler(Scheduler s){\n")
    f.write("        data.resetScheduler(s);\n")
    f.write("    }\n")
    f.write("    }\n\n")
    ...
    f.write("  private int threadID;\n")
    f.write("  private Controller controller;\n");
    f.write("  private int uselen;\n")
    f.write("\n") 
    
    f.write("  TestThread (String name, " + testObjectClass + " q, TraceRecord[] tr,int tid, Controller c) { \n" )
    f.write("    super(name) ; \n") 
    f.write("    this.data = q ; \n") 
    f.write("    this.elemBase = 0 ; \n ") # Integer.parseInt(name) * " + str(tdRange) + " ;  \n") 
    f.write("    this.trace = tr ; \n") 
    f.write("    threadID = tid;\n")
    f.write("    controller = c;\n")
    f.write("    uselen = 0;\n")
    f.write("  } \n\n")
    ...
    f.write("    for(int i = 0,ej = 0;ej < len;){\n")
    f.write("        int opThisBlock = r.nextInt(3)+1;\n")
    f.write("        int[] op = new int[opThisBlock];\n")
    f.write("        for(int j = 0;j < opThisBlock;j++){\n")
    f.write("            op[j] = r.nextInt("+ str(mdNum) +");\n")
    f.write("        }\n")
    f.write("    controller.uploadThreadInfo(threadID,op);\n")
    f.write("    data.setScheduler(controller.getScheduler());\n")
    f.write("    for(int k = i; k < opThisBlock + i;k++){\n")
  
    
  #  f.write("    for (int i = 0; i < len; i++) { \n") 
    f.write("      LinkedList<ArgType> args = new LinkedList<ArgType> () ; \n") 
    f.write("      int mid = op[k-i]; \n")
    f.write("      switch (mid) { \n")
    for i in range(0, mdNum) :
        ...
        if m[2] == "void" :
            f.write("          " + methodCall + " ; \n")
            f.write("          trace[k] = new TraceRecord(args, -1, mid, false) ; \n") 
        elif m[2] == "obj[int]" :
            f.write("          " + m[0] + "__ret = " + methodCall + " ; \n")
            f.write("          if (" + m[0] + "__ret == null) trace[k] = new TraceRecord(args, -1, mid, false) ; \n")
            f.write("          else trace[k] = new TraceRecord(args, Integer.parseInt(" + m[0] \
                        + "__ret.toString()), mid, false ) ; \n")
        else:
            f.write("          " + m[0] + "__ret = " + methodCall + " ; \n")
            f.write("          trace[k] = new TraceRecord(args, " + m[0] + "__ret, mid, false ) ; \n") 
        f.write("        } catch (Exception e) { \n")
        f.write("          trace[k] = new TraceRecord(args, -1, mid, true) ; \n") 
        f.write("        } \n")
        f.write("        break ; \n") 
    f.write("      default :  \n") 
    f.write("      } \n") 
    f.write("    } \n") 
    f.write("    controller.handleEnd(threadID);\n")
    f.write("    i+=opThisBlock;\n")
    f.write("    ej++;\n")
    f.write("    uselen = i;\n")
    f.write("  } \n") 
    f.write("} \n\n")
 
    f.write("    public int getUselen(){\n")
    f.write("        return uselen;\n")
    f.write("    }\n") 
    f.write("}\n\n") 

    f.write("class " + testClassname + " { \n") 
    f.write("  public static void main(String argv[]) { \n")
    f.write("    int tdNum = " + str(test.threadNum) + ", mdNum = " + str(mdNum) + ", trLen = " + str(test.traceLength*3) + "; \n")
    f.write("    " + testObjectClass + " testObj = new " + testObjectClass + "() ; \n") 
    f.write("    Controller ctl = new Controller(tdNum,mdNum);\n")
    ...
    f.write("      thd[i] = new TestThread(Integer.toString(i), testObj, tr[i],i,ctl) ; \n")
    f.write("      thd[i].start() ; \n")
    f.write("    } \n")
    f.write("    try { \n")
    f.write("      for (int i = 0; i < tdNum; i++)  \n")
    ...
    f.write("      out.write(\"# <MethodIndex> <ReturnValue> <ArgumentValue>\\n\") ; \n")
    f.write("      out.write(\"" + str(test.threadNum) + " " + str(test.traceLength) + " " + test.classname)
    for m in test.methods : 
        f.write(" " + m[0]) 
    f.write("\\n\") ; \n")
    
    f.write("        int[] uslen = new int[tdNum];\n")
    f.write("    for(int w= 0;w < tdNum;w++){\n")
    f.write("        uslen[w] = thd[w].getUselen();\n")
    f.write("    }\n")
    ...
    f.write("        for (int j=0; j < uslen[i]; j++) out.write(tr[i][j].toString() + \"\\n\") ; \n")
    f.write("      } \n")
    f.write("      out.close() ; \n")
    f.write("    } catch (Exception e) { \n")
...
def generateSimulateScalaSource(test, testPath, optPath, pkgName, classname): 
    ...
    if test.optimisation == "" :
        ...
        f.write("  type ST = String \n") 
        f.write("  def encodeObject (o: T) = o.toString \n\n")
    else : 
        fopt = open(optPath, "r")
        for line in fopt : 
            f.write("  " + line) 
