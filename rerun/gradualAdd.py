...
for i in range(whichline,whichline+1):
	...
	with open('RerunTrace'+classname+ nameno +'.java','w') as g:
		...
		g.write('package test.'+classname+'AddRemove;\n')
	...
	#	t2 = 500
		for j in range(0,int(lenseq[0])):
			...
			if temp[0] == '0' :
				...
				g.write('\t\ttestobj.vtMethod1('+ temp[1] +');\n')
				if(temp[2] == '0'):
					t1 = t1 - 1
				else:
					t2 = t2 - 1
...
######################################################################################################33
		...
		with open(folder+'book_lockfreelist_add_remove.temp.testlog','r') as f:
			idx = 0
			for line in f.readlines():
				...
				if argu[0] == 'Thread':
					...
					if int(argu[0])==0:
						...
						g.write('\t\ttrace['+str(idx - tdNum1 + t1)+'] = new TraceRecord(args,add_ret,0,false);\n')
					else:
						g.write('\t\tremove_1 = '+argu[2]+';\n')
						g.write('\t\targs.add(new ArgInt(remove_1));\n')
						g.write('\t\tremove_ret = data.vtMethod1(remove_1);\n')
						g.write('\t\ttrace['+str(idx - tdNum1 + t1)+'] = new TraceRecord(args,remove_ret,1,false);\n')
