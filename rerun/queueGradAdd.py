...
lengthofprefix = 0
if whichline != -1:
	with open(folder+'preprocess','w') as pre:
...
for i in range(whichline,whichline+1):
	...
	if i == -1:
		...
		nameno = str(i)
	with open(outputfolder+'RerunTrace'+classname+ nameno +'.java','w') as g:
		
		g.write('package test.'+classname+'OfferPoll;\n')
		...
		g.write('\t}\n')
		g.write('\tpublic void vtMethod0(int x1){data.offer(x1);}\n')
		g.write('\tpublic int vtMethod1(){return data.poll();}\n')	
		g.write('}\n')
#########################################################################################################
...
#########################################################################################################
	...
	#	t2 = 500
		...
		g.write('\t\ttry{\n')
		g.write('\t\t\tFileWriter fw = new FileWriter("tracelog/'+'loc_'+classname+'_'+nameno+'.temp.testlog'+'");\n')
		g.write('\t\t\tBufferedWriter out = new BufferedWriter(fw);\n')
		g.write('\t\t\tout.write("# <MethodIndex> <ArgumentValue> <ReturnValue>\\n");\n')
		g.write('\t\t\tout.write("2 1000 '+classname+' add remove\\n");\n')
...
######################################################################################################33
	...
	#	g.write('\t\tint remove_1;\n')
		g.write('\t\tint poll_ret;\n')
		g.write('\t\tLinkedList<ArgType> args;\n') 
		
		with open(folder+'book_optimisticlinkedqueue_offer_poll.temp.testlog','r') as f:
			idx = 0
			for line in f.readlines():
				...
				#		g.write('\t\tint remove_1;\n')
					...
					if int(argu[0])==0:
						g.write('\t\toffer_1 = '+argu[2]+';\n')
						g.write('\t\targs.add(new ArgInt(offer_1));\n')
						g.write('\t\tdata.vtMethod0(offer_1);\n')
					...
					#	g.write('\t\targs.add(new ArgInt(remove_1));\n')
						g.write('\t\tpoll_ret = data.vtMethod1();\n')
						g.write('\t\ttrace['+str(idx - tdNum1 + t1)+'] = new TraceRecord(args,poll_ret,1,false);\n')
				...
				#		g.write('\t\targs.add(new ArgInt(remove_1));\n')
						g.write('\t\tpoll_ret = data.vtMethod1();\n')
						g.write('\t\ttrace['+str(idx-tdNum1-tdNum2+t2)+'] = new TraceRecord(args,poll_ret,1,false);\n')
					idx = idx + 1
					continue
				else:
					idx = idx + 1
