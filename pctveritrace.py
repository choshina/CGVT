...
def runTesting (test, targetDir, testPathClass, logPath, logName, limit) :
    agentArgs = "=" + logPath + "/" + logName + "," + repr(test.threadNum) + "," + repr(test.traceLength) + "," \
        +"pct_"+ targetDir + "," + test.classname + "," + str(len(test.methods)) 
...
logPath = vtHomePath + "/tracelog/pct"
...
pkgName = "test.pct_" + test.classname
if test.outFile == "" :
...
testTargetPath = "test/" + "pct_"+targetDir
