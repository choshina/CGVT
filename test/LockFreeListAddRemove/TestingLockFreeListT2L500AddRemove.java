...
private Controller controller;
  ...
  TestThread (String name, VTLockFreeListTest q, int id, 
				Controller c, TraceRecord[] tr) { 
    super(name) ; 
    this.data = q ; 
    this.elemBase = 0 ; 
     this.trace = tr ; 
	...
	for(int k = i;(k < opThisBlock+i)&&(k< len);k++){	
      ...
      case 0: 
        ...
        try { 
          ...
          trace[k] = new TraceRecord(args, add__ret, op[k-i], false ) ; 
        } catch (Exception e) {
          trace[k] = new TraceRecord(args, -1, op[k-i], true) ; 
        } 
        break ; 
      case 1: 
        ...
        try { 
          ...
          trace[k] = new TraceRecord(args, remove__ret, op[k-i], false ) ; 
        } catch (Exception e) { 
          trace[k] = new TraceRecord(args, -1, op[k-i], true) ; 
        } 
        break ; 
...
class TestingLockFreeListT2L500AddRemove { 
  public static void main(String argv[]) { 
    ...
    for (int i = 0; i < tdNum; i++) { 
      ...
      thd[i] = new TestThread(Integer.toString(i), testObj,i,controller, tr[i]) ; 
      thd[i].start() ; 
    } 
    try { 
      for (int i = 0; i < tdNum; i++)  
    ...
    try { 
      ...
      out.write("2 500 LockFreeList add remove\n") ; 
      for (int i=0; i < tdNum; i++) { 
        out.write("Thread " + i + "\n") ; 
