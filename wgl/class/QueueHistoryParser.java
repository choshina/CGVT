...
public class QueueHistoryParser extends HistoryParser{

	public QueueHistoryParser(String t, String j) {
		super(t, j);
		...
		while(true){
			...
			if(to.opT == 0){
				dn = new HistoryNode(nodeId,tOpId,
						new QueueOperationEnq(tOpId,to.opT,minIndex,to.arg,to.start,to.end));
			}else if(to.opT == 1){
				dn = new HistoryNode(nodeId,tOpId,
						new QueueOperationDeq(tOpId,to.opT,minIndex,to.ret,to.start,to.end));
			}
