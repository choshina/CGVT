...
public class SetHistoryParser extends HistoryParser{
	...
	protected void formHistory() {
		...
		while(true){
			...
			HistoryNode dn = null;
			if(to.opT == 0||to.opT == 1){
				boolean tRet = 	(to.ret==0?false:true);
				dn = new HistoryNode(nodeId,tOpId,
							new SetOperationARC(tOpId,to.opT,minIndex,to.arg,tRet,to.start,to.end));//set-specific code
			}else if(to.opT == 2){//2->size()
				dn = new HistoryNode(nodeId,tOpId,
							new SetOperationSize(tOpId,to.opT,minIndex,to.ret,to.start,to.end));
			}
